...
function MyShelter({ theme, setShowMyShelter }) {
    ...
    useEffect(() => {
        ...
        // Обработка кликов для перемещения объектов
        const handleClick = (event) => {
            ...
            let maxZIndex = Math.max(...bodiesRef.current.map(b => b.render.zIndex || 0));
            ...
            if (clickedBody) {
                // Перемещаем объект в точку клика
                Matter.Body.setPosition(clickedBody, { x: mouseX, y: mouseY });
                // Устанавливаем самый высокий zIndex
                clickedBody.render.zIndex = maxZIndex + 1;
            }
        };
