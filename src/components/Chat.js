...
function Chat({ userId, room, theme, socket, joinedRoomsRef }) {
  ...
  // Добавляем "Белочку" в список пользователей, если текущая комната — "Парк"
  useEffect(() => {
    if (room === 'Парк') {
      const belochka = {
        userId: 'npc_belochka',
      ...
      setUsers(prevUsers => {
        // Проверяем, есть ли "Белочка" в списке, чтобы избежать дублирования
      ...
      // Удаляем "Белочку" из списка, если комната не "Парк"
      setUsers(prevUsers => prevUsers.filter(user => user.userId !== 'npc_belochka'));
    }
  }, [room]);
  ...
  useEffect(() => {
    ...
    socket.on('roomUsers', (roomUsers) => {
      ...
      // Обновляем пользователей, но сохраняем "Белочку", если комната "Парк"
      const updatedUsers = room === 'Парк'
        ? [roomUsers.find(user => user.userId === 'npc_belochka') || { userId: 'npc_belochka', firstName: 'Белочка', photoUrl: npcBelochkaImage }, ...roomUsers]
        : roomUsers;
